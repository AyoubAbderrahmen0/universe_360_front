...
const AddProduct = (props) => {
    ...
    const handleClick = () => {
        if(name && description && price && imgUrl && sku && category && brand && model && discountPrice && stockQuantity && weight && video){
            dispatch(addProduct({ name, description, price, imgUrl, sku, category, brand, model, discountPrice, stockQuantity, weight, video , clientID:client._id }));
        props.setShow(false)
        }
    };
    const back = () => {
    ...
    return (
        <div className='card m-2'>
            ...
            <Form className='mx-2'>
                ...
                <Button className='my-2 me-2' variant="success" onClick={handleClick}>Save</Button>
                <Button className='my-2' variant="danger" onClick={back}>Back</Button>
                </div>
            </Form>
        </div>
    );
